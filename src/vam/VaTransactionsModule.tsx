  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-500">
      ...
      {approvalQueue.some(a => a.status === 'PENDING') && (
        <div className="bg-amber-50 p-6 rounded-[32px] border border-amber-100 animate-in slide-in-from-top duration-300">
          ...
          {approvalQueue.filter(a => a.status === 'PENDING').map(item => (
            <div key={item.id} className="bg-white p-4 rounded-2xl border border-amber-100 flex items-center justify-between">
              <div>
                <p className="font-black text-slate-800 text-sm">{item.description}</p>
                <p className="text-[10px] text-slate-400 font-bold">{t('approver.submitted_by')}: {item.submittedBy} • Rp {item.amount.toLocaleString()}</p>
              </div>
              <button onClick={() => handleApproveBatch(item.id)} className="px-4 py-2 bg-emerald-50 text-emerald-600 rounded-xl text-[10px] font-black uppercase tracking-widest hover:bg-emerald-600 hover:text-white transition-all">
                {t('approver.approve')}
              </button>
            </div>
          ))}
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
        <div className="bg-white p-6 rounded-[32px] border border-slate-200 shadow-sm">
          <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">{t('tx.count')}</p>
          <p className="text-3xl font-black text-slate-800">{BATCH_HISTORY.reduce((a, b) => a + b.count, 0).toLocaleString()}</p>
          <p className="text-xs text-emerald-500 font-bold mt-1">Total Records</p>
        </div>
        <div className="bg-white p-6 rounded-[32px] border border-slate-200 shadow-sm">
          <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">{t('tx.total_amount')}</p>
          <p className="text-3xl font-black text-slate-800">Rp {(BATCH_HISTORY.reduce((a, b) => a + b.totalAmount, 0) / 1000000000).toFixed(1)}B</p>
        </div>
        <div className="bg-white p-6 rounded-[32px] border border-slate-200 shadow-sm">
          <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">{t('tx.success_rate')}</p>
          <p className="text-3xl font-black text-emerald-600">99.2%</p>
        </div>
        <div className="bg-white p-6 rounded-[32px] border border-slate-200 shadow-sm">
          <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">{t('tx.fee_collected')}</p>
          <p className="text-3xl font-black text-blue-600">Rp {(BATCH_HISTORY.reduce((a, b) => a + b.feeCollected, 0) / 1000000).toFixed(0)}M</p>
        </div>
      </div>
      ...
      <div className="bg-white rounded-[40px] border border-slate-200 shadow-sm overflow-hidden">
        <table className="w-full text-sm">
          <thead className="bg-slate-50 text-left">
            <tr>
              <th className="px-6 py-4 text-[10px] font-black text-slate-400 uppercase tracking-widest">{t('tx.filename')}</th>
              <th className="px-6 py-4 text-[10px] font-black text-slate-400 uppercase tracking-widest">{t('tx.client')}</th>
              <th className="px-6 py-4 text-[10px] font-black text-slate-400 uppercase tracking-widest">{t('tx.status')}</th>
              <th className="px-6 py-4 text-[10px] font-black text-slate-400 uppercase tracking-widest text-center">{t('tx.count')}</th>
              <th className="px-6 py-4 text-[10px] font-black text-slate-400 uppercase tracking-widest text-right">{t('tx.total_amount')}</th>
              <th className="px-6 py-4 text-[10px] font-black text-slate-400 uppercase tracking-widest text-right">{t('tx.fee_collected')}</th>
              <th className="px-6 py-4 text-[10px] font-black text-slate-400 uppercase tracking-widest text-right">{t('tx.date')}</th>
            </tr>
          </thead>
          <tbody className="divide-y divide-slate-100">
